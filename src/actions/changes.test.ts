...
test("only run git changes", async () => {
	...
	fs.rmSync(testPath("template-bun", "a.txt"));
	await changes(allDirs, "touch test2.txt");
	expect(fs.existsSync(testPath("env", "test2.txt"))).toBe(false);
	expect(fs.existsSync(testPath("template-bun", "test2.txt"))).toBe(false);
	...
	expect(fs.existsSync(testPath("template-bun", "change.txt"))).toBe(false);
});
